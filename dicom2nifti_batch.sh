...
usage () { echo "$(basename "$0") [-h] -i input_directory -o output_directory -- program to perform conversion of dcm to nifti
...
FILE_LIST=`find $INPUT_DIR -type d -name "*DTI*" ` #this way it does the conversion to all folders instead of just a few
...
do
   if [[ -n $(find $FILE -maxdepth 3 -type f -name "*Axial*DTI*.nii" -o -name "*Axial*DTI*.json") ]]
    then
        echo "Processing folder ($INDEX/$FILE_LIST_SIZE):"
        echo "The dcm2niix conversion has already been done."
    else  
       #Removes the content of INPUT_DIR and removes the extensions, keeping the "relative path" and file name
        ...
        STARTTIME=`date +%s%N`
        #Run the script
        dcm2niix ${FILE} #-o $OUTPUT_DIR/$RELATIVE_PATH
        ...
        echo "$elapsed,$FILE" >> $OUTPUT_DIR/results.txt
    fi
    #Increment INDEX
    let INDEX=${INDEX}+1
done
